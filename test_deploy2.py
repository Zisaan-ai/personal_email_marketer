...
try:
    ...
    ftp = ftplib.FTP()
    ftp.connect(FTP_HOST, timeout=20)
    ftp.login(FTP_USER, FTP_PASS)
    ftp.set_pasv(False) # try active mode instead of passive? Or just passive. FTP default is passive. Let's just try connect and login again.
