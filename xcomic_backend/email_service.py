...
def log_debug(msg):
    try:
        with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'email_debug.log'), 'a') as f:
            f.write(msg + '\n')
    except:
        pass

def _send_system_email(subject: str, body_html: str, recipient: str) -> bool:
    log_debug(f"Attempting to send email to {recipient}")
    ...
    if not smtp_pass or smtp_pass == "your_app_password_here":
        log_debug("SMTP pass is missing or default")
        return False
        
    try:
        from_name = config.get("SMTP_FROM_NAME", "System Admin")
        log_debug(f"Calling generate_clean_plaintext")
        body_text = generate_clean_plaintext(body_html)
        
        # Brevo API Support for System Emails
        if smtp_pass.startswith("xkeysib-"):
            log_debug(f"Using Brevo API for {recipient}")
            import requests
            headers = {
                "accept": "application/json",
                "api-key": smtp_pass,
            ...
            log_debug("Sending request to Brevo API")
            res = requests.post("https://api.brevo.com/v3/smtp/email", json=payload, headers=headers, timeout=10)
            log_debug(f"Brevo API response: {res.status_code} {res.text}")
            if res.status_code in [200, 201, 202]:
                return True
            return False

        # Standard SMTP
        log_debug("Using standard SMTP")
        ...
        server.send_message(msg)
        log_debug("Standard SMTP sent")
        return True
    except Exception as e:
        log_debug(f"Exception in _send_system_email: {e}")
        import traceback
