        ...
        try {
            ...
            if (res.ok) {
                ...
                if (data.has_account) {
                    ...
                    const provEl  = document.getElementById('smtp-provider');
                    
                    if (hostEl)  hostEl.value  = data.smtp_host     || '';
                    if (portEl)  portEl.value  = data.smtp_port     || 587;
                    if (userEl)  userEl.value  = data.smtp_username || '';
                    if (passEl)  passEl.value  = data.smtp_password || '';
                    if (nameEl)  nameEl.value  = data.from_name     || '';
                    if (emailEl) emailEl.value = data.from_email    || '';
                    if (provEl)  provEl.value  = 'smtp';
                    
                    const smtpFieldsEl = document.getElementById('smtp-fields');
                    if (smtpFieldsEl) smtpFieldsEl.style.display = 'block';
                    
                    statusEl.innerHTML = '&#x2705; <strong>Saved Account:</strong> ' + data.smtp_username + ' &nbsp;|&nbsp; Host: ' + data.smtp_host + ':' + data.smtp_port + ' &nbsp;|&nbsp; Name: ' + (data.from_name || '-');
                    statusEl.className = 'alert success';
                    statusEl.style.display = 'block';
                } else {
                    statusEl.textContent = 'No email account saved yet. Fill the form below and click Save Changes.';
...
window.openColdMailBuilder = async function(id) {
    ...
    window.currentCampaignId = null;
        ...
        // BUG FIX: Reset the action buttons so "Resume" doesn't show for a new campaign
        const sendBtn = document.getElementById('inst-send-seq-btn');
        const actionsDiv = document.getElementById('cold-status-actions');
        if (sendBtn) sendBtn.style.display = 'inline-block';
