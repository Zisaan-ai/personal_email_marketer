...
window.APP_INIT = function() {
    ...
    setupABTest();
    setupAdmin();
    // BUG-06 fix: function is named setupAICopilot in ai_features.js, not setupAIChat
    setupAIChat();
    if (typeof window.setupAICopilot === 'function') window.setupAICopilot();

    // Show AI chat button
    var aiBtn = document.getElementById('ai-chat-btn');
    if (aiBtn) aiBtn.style.display = 'flex';

    // Check pending approvals and show badge
...
function setupCampaignBuilder() {
    ...
    function selectBlock(event, block) {
        ...
        if (type === 'text') {
            ...
            document.getElementById('prop-btn-text').addEventListener('input', e => {
                if (a) a.innerText = e.target.value;
            });
            document.getElementById('prop-btn-url').addEventListener('input', e => {
        ...
        tr.innerHTML = `
            ...
            <td>
                <div style="display:flex; gap:6px; justify-content:flex-end;">
                    ...
                    ${cStatus.toLowerCase() === 'paused' ? `<button class="btn" style="padding:6px 12px;font-size:13px;background:#ecfdf5;border:1px solid #a7f3d0;" onclick="resumeCampaign('${c.id}')" title="Resume"><i class="fa-solid fa-play" style="color:#059669;"></i></button>` : ''}
