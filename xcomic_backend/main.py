...
def save_campaign_options(campaign_id: str, options: CampaignOptionsUpdate, current_user: database.User = Depends(auth.get_current_user), db: Session = Depends(database.get_db)):
    ...
    if not camp:
        raise HTTPException(status_code=404, detail="Campaign not found")
    if options.track_opens is not None:
        camp.track_opens = options.track_opens
    if options.track_clicks is not None:
        camp.track_clicks = options.track_clicks
    if options.use_unsubscribe is not None:
        camp.use_unsubscribe = options.use_unsubscribe
    if options.max_emails_per_day is not None:
...
def get_campaigns(current_user: database.User = Depends(auth.get_current_user), db: Session = Depends(database.get_db)):
    ...
    return [{
        ...
        "use_unsubscribe": c.use_unsubscribe,
...
def resume_campaign(campaign_id: str, background_tasks: BackgroundTasks, current_user: database.User = Depends(auth.get_current_user), db: Session = Depends(database.get_db)):
    ...
    if campaign.status in ["paused", "failed", "completed"]:
        from sqlalchemy import text
        res = db.execute(text("UPDATE campaigns SET status='processing' WHERE id=:id AND status IN ('paused', 'failed', 'completed')"), {"id": str(campaign.id)})
        db.commit()
        if res.rowcount > 0:
...
def delete_campaign_lead(campaign_id: str, lead_id: str, current_user: database.User = Depends(auth.get_current_user), db: Session = Depends(database.get_db)):
    ...
    if campaign.user_id != str(current_user.id) and not current_user.is_admin:
        raise HTTPException(status_code=403, detail="Not authorized")
    lead = db.query(database.CampaignLead).filter(database.CampaignLead.id == lead_id, database.CampaignLead.campaign_id == campaign_id).first()
    if not lead:
...
def _run_campaign(db, campaign_id):
    ...
    def send_to_lead(db_session, lead, variant_label=None):
        ...
        if campaign_user:
            ...
            if plan == "free" and sent_count >= 10:
                ...
                c.status = "paused"
                db_session.commit()
                print(f"Campaign {campaign_id} paused due to Starter Tier limit (1,000 emails).")
                return False
            elif plan == "professional" and sent_count >= 10000:
                c.status = "paused"
                db_session.commit()
                print(f"Campaign {campaign_id} paused due to Professional Tier limit (10,000 emails).")
                return False
    ...
    if c and c.status != "paused":
        all_leads = list(db.query(database.CampaignLead).filter(database.CampaignLead.campaign_id == campaign_id).all())
        active_count = sum(1 for l in all_leads if l.status in ("pending", "processing"))
        if active_count == 0:
            c.status = "completed"
        else:
...
def create_sending_account(acc: SendingAccountCreate, current_user: database.User = Depends(auth.get_current_user), db: Session = Depends(database.get_db)):
    ...
    if user_plan == "free" and acc_count >= 2:
        ...
        raise HTTPException(status_code=403, detail="ENTERPRISE_LIMIT_REACHED: Enterprise plan allows maximum 50 sending accounts.")
...
def check_spam_score_endpoint(req: SpamCheckRequest, current_user: database.User = Depends(auth.get_current_user), db: Session = Depends(database.get_db)):
...
def test_smtp_settings(req: Optional[SMTPTestRequest] = None, current_user: database.User = Depends(auth.get_current_user)):
    ...
    try:
        ...
        msg = MIMEText("This is a test email from XComic to verify your SMTP settings. If you received this, your email delivery is working!", "plain")
        msg["Subject"] = "XComic SMTP Test Successful"
        msg["From"] = smtp_user
        
        # Check if test_email is provided in req
        to_email = getattr(req, "test_email", current_user.email) if hasattr(req, "test_email") and getattr(req, "test_email") else current_user.email
        msg["To"] = to_email
        
        server.sendmail(smtp_user, to_email, msg.as_string())
        server.quit()
        
        return {"ok": True, "message": f"SMTP test email sent successfully to {to_email}!"}
    except Exception as e:
...
def get_admin_notifications_unread(current_user: database.User = Depends(auth.get_current_user), db: Session = Depends(database.get_db)):
    if not current_user.is_admin:
        raise HTTPException(status_code=403, detail="Admin only")
    ...
    # 3. Paid subscriptions count (active non-free subscriptions)
    paid_subs_count = db.query(database.User).filter(database.User.subscription_plan != "free", database.User.subscription_plan != None).count()

    total_admin_unread = support_count + new_users_count

    return {
        "support_count": support_count,
        "new_users_count": new_users_count,
        "paid_subs_count": paid_subs_count,
        "total_admin_unread": total_admin_unread,
        "count": support_count
    }
