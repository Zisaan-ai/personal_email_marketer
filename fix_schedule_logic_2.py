...
# 1. Add _auto_resume_stuck_campaigns and _scheduler_start_scheduled_campaigns and is_campaign_within_schedule
new_functions = """
...
if "def is_campaign_within_schedule" not in content:
    content = content.replace("from bounce_processor import check_bounces", new_functions)
...
"""
if "auto_resume_job" not in content:
...
if "is_campaign_within_schedule(campaign)" not in content:
    content = content.replace("# BUG FIX: Build unsubscribe set for fast lookup", run_campaign_check)
