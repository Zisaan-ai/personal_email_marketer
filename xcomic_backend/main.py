    ...
    try:
        ...
        if needs_reset:
            for acc in needs_reset:
                ...
                # Reset and increment warmup limits
            ...
            db.commit()
            print(f"[Daily Reset] Reset sent_today and warmup limits for {len(needs_reset)} accounts (BD date: {today_bd})")
    except Exception as e:
        db.rollback()
...
def trigger_warmup_manual(current_user: database.User = Depends(auth.get_current_user)):
