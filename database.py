...
class BulkCampaign(Base):
    ...
    sending_account_ids = Column(String, nullable=True) # JSON list of account IDs
    scheduled_at = Column(DateTime, nullable=True)
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)

class BulkCampaignLead(Base):
    __tablename__ = "bulk_campaign_leads"

    id = Column(String, primary_key=True, default=generate_uuid)
    bulk_campaign_id = Column(String, ForeignKey("bulk_campaigns.id"), index=True)
    email = Column(String, index=True)
    status = Column(String, default="pending") # pending, sent, failed, bounced
    sent_at = Column(DateTime, nullable=True)
    sending_account_id = Column(String, nullable=True) # ID of account that actually sent this email
