...
scheduler.add_job(warmup_service.run_warmup_cycle, 'interval', minutes=30, id='warmup_job', coalesce=True, max_instances=1, misfire_grace_time=300)
...
# ΓöÇΓöÇΓöÇ TIMEZONE-AWARE AUTO RESET ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
# Bangladesh = UTC+6. Reset sent_today when Bangladesh date changes.
# This runs on every sending_accounts API call ΓÇö works even if server restarts.
    ...
    try:
        ...
        if needs_reset:
            for acc in needs_reset:
                ...
                acc.warmup_daily_limit = min((acc.warmup_daily_limit or 5) + (acc.warmup_increment_per_day or 2), 50)
            db.commit()
...
def delete_user(user_id: str, current_user: database.User = Depends(auth.get_current_user), db: Session = Depends(database.get_db)):
    ...
    if not target_user:
        raise HTTPException(status_code=404, detail="User not found")
    # Protect the owner account ├óΓé¼ΓÇ¥ can NEVER be deleted
    ...
    def _run_seed_test():
        ...
        try:
            ...
            if req.seed_imap_server and req.seed_imap_password:
                ...
                # No IMAP credentials ΓÇö mark as sent (user checks manually)
                t = test_db.query(database.SeedTestResult).filter(database.SeedTestResult.id == test_id).first()
                if t:
