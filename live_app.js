...
window.viewAnalytics = function(id) {
    ...
    if (c.type === 'cold_mail') {
        ...
        // BUG-2 FIX: was 'cold-mail' which doesn't exist â correct ID is 'cold-mail-builder'
            ...
            if (container && wrapper && healthData.length > 0) {
                ...
                healthData.forEach(h => {
                    ...
                    const autoPauseBanner = h.auto_paused 
                        ...
                        ? `<div style="background:#fef2f2;color:#dc2626;padding:6px 10px;border-radius:6px;font-size:11px;margin-top:8px;">â ï¸ Auto-paused: ${h.auto_paused_reason || 'Poor health'}</div>` 
                    ...
                    container.innerHTML += `
                        ...
                        <div style="background:#fff;padding:20px;border-radius:12px;border:1px solid rgba(0,0,0,0.06);box-shadow:0 2px 8px rgba(0,0,0,0.02);transition:transform 0.2s;" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform='translateY(0)'">
                            ...
                            <div style="display:flex;gap:12px;font-size:12px;color:var(--text-muted);">
                                ...
                                <span>ð§ ${(h.metrics || {}).total_sent || 0} sent</span>

                                <span>ð¥ ${(h.metrics || {}).bounce_rate || 0}% bounce</span>

                                <span>ð ${h.suggested_daily_limit || '-'}/day</span>
...
window.saveSchedule = async function() {
    ...
    try {
        ...
        if (res && res.ok) {
            ...
            showToast('â Schedule saved successfully!', 'success');
        ...
        saveGeminiBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (geminiStatus) {
                    ...
                    geminiStatus.textContent = 'â AI Keys saved successfully!';
            ...
            if (type === 'text') {
                ...
                <div style="text-align:center;padding:30px 20px;background:#f1f5f9;">
                    ...
                    <p style="font-family:Helvetica,Arial,sans-serif;font-size:12px;color:#64748b;line-height:1.5;margin-bottom:10px;">
                        ...
                        Copyright Â© 2026 Your Company, All rights reserved.<br>
...
<body style="margin:0;padding:0;background-color:#F6F6F4;font-family:Helvetica,Arial,sans-serif;">
    ...
    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color:#F6F6F4;padding:20px 0;">
        ...
        try {
            ...
            if (res.ok) {
                ...
                showToast('Campaign sent successfully! ð', 'success');
    ...
    function loadTemplate(templateId) {
        ...
        showToast('â¨ Template loaded: ' + tmpl.name, 'success');
...
function setupSequenceBuilder() {
    ...
    if (sendSeqBtn) sendSeqBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            if (res.ok) showToast('Campaign launched successfully! ð', 'success');
    ...
    try {
        ...
        users.forEach(u => {
            ...
            const statusBadge = u.is_approved
                ...
                ? `<span style="background:#dcfce7;color:#059669;padding:3px 10px;border-radius:20px;font-size:11px;font-weight:700;">â Approved</span>`

                : `<span style="background:#fef9c3;color:#ca8a04;padding:3px 10px;border-radius:20px;font-size:11px;font-weight:700;">â³ Pending</span>`;
...
window.approveUser = async function(id) {
    ...
    try {
        ...
        if (res.ok) { 
            ...
            showToast('User approved successfully! â', 'success'); 
    ...
    el.addEventListener('drop', (e) => {
        ...
        if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
            ...
            if (file.name.endsWith('.csv') || file.name.endsWith('.txt')) {
                    ...
                    try {
                        ...
                        if (res.ok) {
                            ...
                            if (removed > 0) {
                                ...
                                showToast(`â ï¸ Removed ${removed} invalid/inactive leads. Loaded ${validLines.length} valid leads.`, 'error');

                            } else {

                                showToast(`â Successfully loaded ${validLines.length} healthy leads!`, 'success');
                        ...
                        showToast(`â Loaded ${lines.length} leads (validation skipped)`, 'success');
                ...
                showToast("â ï¸ Please drop a valid .csv or .txt file.");
